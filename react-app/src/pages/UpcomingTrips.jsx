            ...
            <div className="flex items-center justify-center gap-sm mt-md">
              <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md p-2 rounded-full border border-white/20">
                <button 
                  ...
                  className={`px-5 py-2 rounded-full font-semibold transition-all shadow-sm cursor-pointer ${filter === 'All' ? 'bg-primary text-white' : 'text-white/90 hover:bg-white/20'}`}
                >
                  All Trips
                </button>
                <button 
                  onClick={() => setFilter('1N/2D')} 
                  className={`px-5 py-2 rounded-full font-semibold transition-all shadow-sm cursor-pointer ${filter === '1N/2D' ? 'bg-primary text-white' : 'text-white/90 hover:bg-white/20'}`}
                >
                  1N/2D
                </button>
                <button 
                  onClick={() => setFilter('2N/3D')} 
                  className={`px-5 py-2 rounded-full font-semibold transition-all shadow-sm cursor-pointer ${filter === '2N/3D' ? 'bg-primary text-white' : 'text-white/90 hover:bg-white/20'}`}
                >
                  2N/3D
                </button>
              </div>
        ...
        <section className="py-24 px-4 md:px-8 max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-md lg:gap-lg">
            {filteredTrips.map(trip => (
              <div key={trip.id} className="group bg-surface-container-lowest rounded-[1.5rem] overflow-hidden border border-outline-variant/20 transition-all duration-300 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 flex flex-col hover:-translate-y-2 transform">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <p className="text-on-surface-variant font-body-md text-body-md mb-6 flex-grow">{trip.desc}</p>
                  <div className="flex justify-between items-center mt-auto pt-4 border-t border-outline-variant/10">
                    <div>
                      <span className="text-label-sm font-label-sm text-outline block uppercase tracking-wider text-[10px]">Starting from</span>
                      <span className="text-headline-md font-bold text-primary">₹{trip.price.toLocaleString('en-IN')}</span>
                    </div>
                    <Link to={`/checkout?trip=${encodeURIComponent(trip.name)}&price=${trip.price}`} className="bg-primary-container hover:bg-primary text-on-primary-container hover:text-white px-5 py-2.5 rounded-xl transition-all font-semibold active:scale-95">Book Now</Link>
                  </div>
                </div>
              </div>
            ))}
