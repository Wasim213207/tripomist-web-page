...
const BookingModal = ({ isOpen, onClose, tripTitle, price, travellers, destination, packageId }) => {
      ...
      <div className="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl animate-fade-in relative max-h-[90vh] overflow-y-auto">
        ...
        {success ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <button onClick={onClose} className="w-full bg-[#136b8a] hover:bg-[#0f556e] text-white font-bold py-3.5 rounded-xl shadow-md transition-all active:scale-[0.98]">
              Done
            </button>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="relative">
                ...
                <div onClick={() => !loading && setIsCalendarOpen(!isCalendarOpen)} className={`w-full border border-gray-200 rounded-xl px-4 py-2.5 flex justify-between items-center transition-colors ${loading ? 'bg-gray-50 text-gray-400 cursor-not-allowed' : 'bg-white text-gray-700 hover:border-[#136b8a] cursor-pointer'}`}>
              ...
              <div>
                ...
                <textarea value={formData.specialRequest} onChange={(e) => setFormData({ ...formData, specialRequest: e.target.value })} disabled={loading} rows={2} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-[#136b8a] outline-none text-gray-700 disabled:bg-gray-50 resize-none" placeholder="Any specific requirements..." />
              </div>
              ...
              <button type="submit" disabled={loading} className="w-full bg-[#136b8a] hover:bg-[#0f556e] disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-bold py-3.5 rounded-xl shadow-md transition-all active:scale-[0.98] mt-2 flex items-center justify-center">
                {loading ? (
                  <>
