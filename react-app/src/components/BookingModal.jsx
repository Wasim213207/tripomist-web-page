...
const BookingModal = ({ isOpen, onClose, tripTitle, price, travellers, navigate, destination, packageId }) => {
  // Step management: 0 = form, 1 = OTP verification, 2 = sharing & payment
  const [step, setStep] = useState(0);
  const [generatedOtp, setGeneratedOtp] = useState('');
  const [otpInput, setOtpInput] = useState('');
  const [sharing, setSharing] = useState('double'); // default sharing type
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate basic fields before proceeding to OTP
    ...
    // Generate a 6‑digit OTP and move to verification step
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    setGeneratedOtp(otp);
    console.log('Generated OTP (for demo):', otp); // In production, send via SMS/Email
    setStep(1);
    setLoading(false);
  };
  ...
  const verifyOtp = () => {
    ...
    if (otpInput === generatedOtp) {
      setStep(2); // Proceed to sharing & payment selection
    } else {
  ...
  // Handle final payment and booking insertion
  const handlePayment = async () => {
    ...
    const options = {
      ...
      currency: 'INR',
      name: 'Tripomist Booking',
      description: tripTitle,
      handler: async function (response) {
        // On successful payment, insert booking record
        try {
          ...
          console.error('Booking insertion error after payment:', err);
          ...
          <>
            ...
            {step === 1 && (
              <div className="flex flex-col gap-4">
                ...
                <p className="text-sm text-gray-600">Enter the 6‑digit OTP sent to your phone/email.</p>
                <input
                  type="text"
                  value={otpInput}
                  onChange={(e) => setOtpInput(e.target.value)}
                  maxLength={6}
                  className="w-full border border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-[#136b8a] outline-none text-gray-700 disabled:bg-gray-50"
                  placeholder="Enter OTP"
                  disabled={loading}
                />
                {error && (
                  <div className="mb-2 p-3 bg-red-50 border border-red-200 text-red-600 rounded-xl text-sm">
                    {error}
                  </div>
                )}
                <button
                  ...
                  onClick={verifyOtp}
                  disabled={loading}
                  className="w-full bg-[#136b8a] hover:bg-[#0f556e] text-white font-bold py-3.5 rounded-xl shadow-md transition-all disabled:opacity-50"
                >
                  {loading ? 'Verifying...' : 'Verify OTP'}
                </button>
              </div>
            )}
            {/* Step 2: Sharing Selection */}
            {step === 2 && (
              <div className="flex flex-col gap-4">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">Select Sharing Type</h2>
                <select
                  value={sharing}
                  onChange={(e) => setSharing(e.target.value)}
                  disabled={loading}
                  className="w-full border border-gray-200 rounded-xl px-4 py-5 focus:ring-2 focus:ring-[#136b8a] outline-none text-gray-700 disabled:bg-gray-50"
                >
                ...
                <button
                  ...
                  onClick={() => setStep(3)}
                  disabled={loading}
                  className="w-full bg-[#136b8a] hover:bg-[#0f556e] text-white font-bold py-3.5 rounded-xl shadow-md transition-all disabled:opacity-50 mt-4"
                >
